...
export class DashboardWsServer {
    ...
    private _broadcastToClients(payload: string): void {
        for (const ws of this.clients) {
            ...
            // Backpressure guard: if the client's send buffer is congested,
            // drop this message rather than queuing indefinitely.
            // For a live "now" view, a dropped data point is acceptable.
            if (ws.bufferedAmount > BACKPRESSURE_THRESHOLD_BYTES) {
                logger.warn('[DashboardWS] Client buffer full — dropping message', {
